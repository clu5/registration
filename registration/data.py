...
def make_datasets(root, img, seg, combine=False):
    root = Path(root)
    ...
    if combine:
        # key = pd.concat(list(datasets.keys()))
        values = list(chain(*sets))
        return RegistrationDataset(data=values)
    else:
...
def make_msd_datasets(
    ...
    return make_datasets(root=root, img=img, seg=seg, combine=combine)
